...
def prepare_image(image, image_dims):
...
@app.route("/")
def homepage():
    return "Welcome to crop diseases detection application!"
...
def predict():
    ...
    if flask.request.method == "POST":
        if flask.request.files.get("image"):
            ...
            image = prepare_image(image, (settings.IMAGE_HEIGHT, settings.IMAGE_WIDTH))
            ...
            while True:
                ...
                # sleep for a samll amount to give the model a chance to classify the input image
                time.sleep(settings.CLIENT_SLEEP)

            # indicate that the request was a sucess
            data["success"] = True
...
# for debugging purpose, it's helpful to start the Flask testing server (don't use this for production)
